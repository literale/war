...
namespace war
{
    class Ill
    {
        ...
        //Данная функция предназначена для создания новой вариации болезни, которая появляется при
        // передачи болезни от одной особи другой. Случайно выбранный параметр меняется на случайно выбранную
        // велечину из прмежутка [-MV; MV], где MV - величина мутации, заданная статически
        //Функция вызывается следующим образом: Ill.Mutate()
        ...
        //Данная функция предназначена для передачи болезни от одной особи к другой.
        //Для этого функция создает копию материнской болезни, а затем мутирует ее. Функция возвращает итоговый вариант болезни
        //Функция вызывается следующим способом: Особь2.ILL = Особь1.ILL.GetIll()
